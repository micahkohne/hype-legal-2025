...
/**
 * JCOGS Image Pro - GD-Specific Mean Removal Filter
 * =================================================
 * High-performance mean removal implementation using GD's native imagefilter
 * 
...
/**
 * GD-Specific Mean Removal Filter Class
 * 
 * Uses PHP's native imagefilter function with IMG_FILTER_MEAN_REMOVAL
 * for optimal performance when working with GD image resources.
 */
class MeanRemoval
{
    /**
     * Apply GD-native mean removal filter
     * 
     * Uses imagefilter with IMG_FILTER_MEAN_REMOVAL for maximum performance.
     * 
    ...
    public function apply(ImageInterface $image, array $params = []): ImageInterface
    {
        ...
        if (!is_resource($gd_resource) && !is_object($gd_resource)) {
            throw new \RuntimeException('Invalid GD resource for mean_removal filter');
        }
        
        // Apply GD mean removal filter
        imagefilter($gd_resource, IMG_FILTER_MEAN_REMOVAL);
