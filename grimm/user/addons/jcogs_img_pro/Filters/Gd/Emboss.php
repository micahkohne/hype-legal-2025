...
/**
 * JCOGS Image Pro - GD-Specific Emboss Filter
 * ===========================================
 * High-performance emboss implementation using GD's native imagefilter
 * 
...
/**
 * GD-Specific Emboss Filter Class
 * 
 * Uses PHP's native imagefilter function with IMG_FILTER_EMBOSS
 * for optimal performance when working with GD image resources.
 */
class Emboss
{
    /**
     * Apply GD-native emboss filter
     * 
     * Uses imagefilter with IMG_FILTER_EMBOSS for maximum performance.
     * 
     * @param ImageInterface $image Source image (must be GD image)
     * @param array $params Filter parameters (unused for emboss)
    ...
    public function apply(ImageInterface $image, array $params = []): ImageInterface
    {
        ...
        if (!is_resource($gd_resource) && !is_object($gd_resource)) {
            throw new \RuntimeException('Invalid GD resource for emboss filter');
        }
        
        // Apply GD emboss filter
        imagefilter($gd_resource, IMG_FILTER_EMBOSS);
