...
/**
 * JCOGS Image Pro - GD-Specific Edgedetect Filter
 * ===============================================
 * High-performance edge detection implementation using GD's native imagefilter
 * 
...
/**
 * GD-Specific Edgedetect Filter Class
 * 
 * Uses PHP's native imagefilter function with IMG_FILTER_EDGEDETECT
 * for optimal performance when working with GD image resources.
 */
class Edgedetect
{
    /**
     * Apply GD-native edge detection filter
     * 
     * Uses imagefilter with IMG_FILTER_EDGEDETECT for maximum performance.
     * 
     * @param ImageInterface $image Source image (must be GD image)
     * @param array $params Filter parameters (unused for edgedetect)
    ...
    public function apply(ImageInterface $image, array $params = []): ImageInterface
    {
        ...
        if (!is_resource($gd_resource) && !is_object($gd_resource)) {
            throw new \RuntimeException('Invalid GD resource for edgedetect filter');
        }
        
        // Apply GD edge detection filter
        imagefilter($gd_resource, IMG_FILTER_EDGEDETECT);
