...
namespace BoldMinded\Carson\Dependency\Litzinger\Basee\Update;
...
abstract class AbstractUpdate
{
    ...
    protected function removeHooks($class, $hooks = [])
    {
        ...
        ee()->db->where_in('hook', $hooks)->where('class', $class)->delete('extensions');
    }
    /**
     * @param $class
     * @param array $methods
    ...
    protected function removeActions($class, $methods = [])
    {
        ...
        ee()->db->where_in('method', $methods)->where('class', $class)->delete('actions');
    }
    /**
     * @return array
     */
    ...
    {
        $diff = \array_diff(\array_keys($this->getHookTemplate()), array('class', 'settings', 'priority', 'version', 'enabled'));
        if (!empty($diff)) {
