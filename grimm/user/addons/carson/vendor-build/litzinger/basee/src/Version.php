...
class Version
{
    ...
    {
        $response = ee('Curl')->get($requestUrl, ['CURLOPT_CONNECTTIMEOUT' => 3, 'CURLOPT_HTTPHEADER' => ['Cache-Control: no-cache'], 'CURLOPT_RETURNTRANSFER' => \true])->exec();
    ...
    public function fetchLatest()
    {
        ...
        $cachedResponse = $this->cache->get($cacheKey, \true);
        if (!$cachedResponse) {
            try {
                $response = $this->makeRequest($url);
                $this->cache->save($cacheKey, $response, null, \true);
                return $response;
            } catch (\Exception $e) {
                // Fail silently
            }
        ...
        if (!$cachedResponse) {
            try {
                ...
                if (isset($response->versions)) {
                    $this->cache->save($cacheKey, $response->versions, null, \true);
