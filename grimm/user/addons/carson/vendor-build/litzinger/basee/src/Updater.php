...
use BoldMinded\Carson\Dependency\Litzinger\Basee\Update\AbstractUpdate;
...
class Updater
{
    ...
    public function shouldUpdate(string $addonName = '', bool $redirect = \false) : bool
    {
        ...
        if (isset($installed[$addonName]['module_version']) && \version_compare($installed[$addonName]['module_version'], $addon->getVersion(), '<')) {
    ...
    public function fetchUpdates($currentVersion = '0', $fetchAll = \false)
    {
        ...
        $files = \iterator_to_array(new RecursiveDirectoryIterator($this->filePath, FilesystemIterator::SKIP_DOTS | FilesystemIterator::UNIX_PATHS), \true);
        \ksort($files);
        foreach ($files as $file) {
            ...
            if (\preg_match('/^up_0*(\\d+)_0*(\\d+)_0*(\\d+).php$/', $fileName, $m)) {
                ...
                if (\version_compare($fileVersion, $currentVersion, '>')) {
                    $remainingUpdates++;
                    if ($fetchAll === \true || !$nextUpdate || \version_compare($fileVersion, $nextUpdate, '>')) {
    ...
    public function runUpdates()
    {
        foreach ($this->updateFiles as $file) {
            ...
            $update->setHookTemplate($this->getHookTemplate())->doUpdate();
        }
    }
    /**
