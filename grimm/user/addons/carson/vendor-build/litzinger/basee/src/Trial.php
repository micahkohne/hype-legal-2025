...
class Trial
{
    ...
    {
        $alert = ee('CP/Alert');
        $alert->makeStandard()->asImportant()->withTitle($this->getMessageTitle())->addToBody($this->getMessageBody())->now();
    }
    public function showTrialExpiredInline()
    {
        if (REQ === 'CP') {
            $alert = ee('CP/Alert');
            return $alert->makeInline()->asImportant()->withTitle($this->getMessageTitle())->addToBody($this->getMessageBody())->render();
        } else {
