...
class Setting
{
     ...
     * @var array
     */
    private $globalSettings = ['installed_date' => '', 'installed_version' => '', 'installed_build' => '', 'license' => ''];
    /**
    ...
    public function load()
    {
        ...
        foreach ($settingsQuery->result() as $row) {
            ...
            if ($row->type == 'json') {
                $dbSettings[$row->site_id][$row->key] = \json_decode($row->val, \true);
    ...
    public function createTable()
    {
        ...
        ee()->load->dbforge();
        ee()->dbforge->add_field(['id' => ['type' => 'int', 'constraint' => 10, 'unsigned' => \true, 'auto_increment' => \true], 'site_id' => ['type' => 'int', 'constraint' => 4, 'unsigned' => \true, 'default' => 1], 'key' => ['type' => 'varchar', 'constraint' => 32], 'val' => ['type' => 'text'], 'type' => ['type' => 'char', 'constraint' => 7]]);
        ee()->dbforge->add_key('id', \true);
