...
namespace BoldMinded\Carson\Dependency\Litzinger\Basee;
 ...
 * @license     MIT
 */
use BoldMinded\Carson\Dependency\Litzinger\Basee\Helper\StringHelper;
use BoldMinded\Carson\Dependency\Litzinger\Basee\Model\SaveResponse;
use Closure;
use BoldMinded\Carson\Dependency\EllisLab\ExpressionEngine\Service\Sidebar\FolderItem;
use BoldMinded\Carson\Dependency\EllisLab\ExpressionEngine\Service\Sidebar\FolderList;
use BoldMinded\Carson\Dependency\EllisLab\ExpressionEngine\Service\Sidebar\Sidebar;
use BoldMinded\Carson\Dependency\EllisLab\ExpressionEngine\Service\View\View;
/**
...
abstract class Controller
{
    ...
    protected function __construct()
    {
        ...
        $this->vars = ['base_url' => $this->createPageUrl(), 'save_btn_text' => 'btn_save_settings', 'save_btn_text_working' => 'btn_saving'];
    }
    /**
     * @return $this
     */
    protected function loadModuleStyles()
    {
        ee()->cp->add_to_head(\sprintf('
            <link href="%s%s/styles/module.css" rel="stylesheet" />', URL_THIRD_THEMES, $this->addonName));
        return $this;
    }
    /**
     * @return string
            ...
            if ($sectionUrl == $seg4 || $sectionUrl == $seg4 . '/' . $seg5) {
                ...
                foreach ($children as $langKey => $child) {
                    ...
                    if ($list instanceof FolderList) {
                        ...
                        if (isset($child['cannotRemove']) && $child['cannotRemove'] === \true) {
                            $item->cannotRemove();
                        }
                    }
    ...
    protected function handleSubmit()
    {
        ...
        if (ee()->form_validation->run() !== \false || $this->bypassValidation() && !empty($_POST)) {
            ...
            if ($callbackResponse) {
                $alert = ee('CP/Alert');
                $alert->makeInline('shared-form')->asSuccess()->withTitle($callbackResponse->getSaveSuccessTitle())->addToBody(\vsprintf($callbackResponse->getSaveSuccessBody(), $callbackResponse->getMessageParameters() ?? []))->defer();
                ...
                if ($callbackResponse->getSaveSuccessUrl()) {
                    ...
                    if ($callbackResponse->getEntityId()) {
            ...
            $alert = ee('CP/Alert');
            $alert->makeInline('shared-form')->asIssue()->withTitle('Please correct the following errors.')->addToBody($errors)->defer();
            ee()->functions->redirect($this->currentPageUrl());
        }
        return $this;
    }
    ...
    protected function parseView($view = null)
    {
        ...
        $params = ['body' => $view->render($vars), 'breadcrumb' => [$this->moduleHomeUrl()->compile() => lang(\sprintf('%s_module_name', $this->addonName))]];
    ...
    protected function buildSettingsFields(array $fieldOptions = [], $values = [])
    {
        ...
        foreach ($fieldOptions as $key => $options) {
            ...
            $field = ['title' => $options['title'], 'fields' => [$key => ['type' => $options['type'], 'value' => $value]]];
