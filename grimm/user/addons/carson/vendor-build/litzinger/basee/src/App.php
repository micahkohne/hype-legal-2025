...
class App
{
    ...
    public static function isFeatureAvailable(string $featureName) : bool
    {
        $features = [
            ...
            'fileGrid' => '5.1',
            'entryEditList' => '6.0',
            // Improved entry edit list screen
            'jumpMenu' => '6.0',
            'cloning' => '6.2.5',
            'revisionAlert' => '6.4',
            'newFileManager' => '7.1',
    ...
    public static function parseTagPair(string $tagdata, string $var, string $prefix = '') : array
    {
        ...
        foreach ($pairs as $pair) {
            $found[] = ['modifier' => $pair[0], 'content' => $pair[1], 'params' => $pair[2], 'chunk' => $pair[3]];
        }
        return $found;
    }
    /**
    ...
    public static function config($name, $siteId = null)
    {
        ...
        if (self::isLtEE6()) {
            $prefs = ee('db')->select('site_system_preferences')->where('site_id', $siteId)->get('sites')->row('site_system_preferences');
        ...
        $pref = ee('db')->select('value')->where('site_id', $siteId)->where('key', $name)->get('config');
        if ($pref->num_rows() !== 1) {
            return null;
        }
    ...
    public static function isUninstallingAddon(string $addonName) : bool
    {
        ...
        // Legacy
        return isset($_POST['bulk_action']) && $_POST['bulk_action'] === 'remove' && isset($_POST['selection']) && !empty($_POST['selection']) && \in_array($addonName, $_POST['selection']);
    }
    /**
