...
class License
{
    ...
    {
        try {
            $response = ee('Curl')->post($this->licenseCheckUrl, ['CURLOPT_CONNECTTIMEOUT' => 3, 'CURLOPT_HTTPHEADER' => ['Cache-Control: no-cache'], 'CURLOPT_RETURNTRANSFER' => \true, 'payload' => $payload['payload']])->exec();
    ...
    public function validate()
    {
        ...
        if (self::DEBUG || $ping->shouldPing()) {
            $response = $this->checkLicense(['payload' => \base64_encode(\json_encode($this->payload))]);
            ...
            if ($response !== null && isset($response['status']) && (!$this->payload['l'] || \in_array($response['status'], self::STATUSES))) {
