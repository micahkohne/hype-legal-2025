...
class UriTemplateHttpClient implements HttpClientInterface, ResetInterface
{
    ...
    {
        if (\class_exists(\BoldMinded\Carson\Dependency\GuzzleHttp\UriTemplate\UriTemplate::class)) {
            return \BoldMinded\Carson\Dependency\GuzzleHttp\UriTemplate\UriTemplate::expand(...);
        }
        if (\class_exists(\BoldMinded\Carson\Dependency\League\Uri\UriTemplate::class)) {
            return static fn(string $url, array $vars): string => (new \BoldMinded\Carson\Dependency\League\Uri\UriTemplate($url))->expand($vars);
        }
        if (\class_exists(\BoldMinded\Carson\Dependency\Rize\UriTemplate::class)) {
            return (new \BoldMinded\Carson\Dependency\Rize\UriTemplate())->expand(...);
        }
