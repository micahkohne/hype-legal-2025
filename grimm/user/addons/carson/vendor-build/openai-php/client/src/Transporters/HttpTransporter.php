...
namespace BoldMinded\Carson\Dependency\OpenAI\Transporters;

use Closure;
use BoldMinded\Carson\Dependency\GuzzleHttp\Exception\ClientException;
use JsonException;
use BoldMinded\Carson\Dependency\OpenAI\Contracts\TransporterContract;
use BoldMinded\Carson\Dependency\OpenAI\Enums\Transporter\ContentType;
...
final class HttpTransporter implements TransporterContract
{
    ...
    public function requestObject(Payload $payload) : Response
    {
        ...
        $response = $this->sendRequest(fn(): \BoldMinded\Carson\Dependency\Psr\Http\Message\ResponseInterface => $this->client->sendRequest($request));
    ...
    private function throwIfJsonError(ResponseInterface $response, string|ResponseInterface $contents) : void
    {
        ...
        try {
            ...
            $response = \json_decode($contents, \true, flags: \JSON_THROW_ON_ERROR);
