...
namespace BoldMinded\Carson\Dependency\OpenAI\Responses\FineTunes;
...
final class RetrieveResponse implements ResponseContract, ResponseHasMetaInformationContract
{
    ...
    {
        $events = \array_map(fn(array $result): RetrieveResponseEvent => RetrieveResponseEvent::from($result), $attributes['events'] ?? []);
        $resultFiles = \array_map(fn(array $result): RetrieveResponseFile => RetrieveResponseFile::from($result), $attributes['result_files']);
        $validationFiles = \array_map(fn(array $result): RetrieveResponseFile => RetrieveResponseFile::from($result), $attributes['validation_files']);
        $trainingFiles = \array_map(fn(array $result): RetrieveResponseFile => RetrieveResponseFile::from($result), $attributes['training_files']);
    ...
    {
        return ['id' => $this->id, 'object' => $this->object, 'model' => $this->model, 'created_at' => $this->createdAt, 'events' => \array_map(static fn(RetrieveResponseEvent $result): array => $result->toArray(), $this->events), 'fine_tuned_model' => $this->fineTunedModel, 'hyperparams' => $this->hyperparams->toArray(), 'organization_id' => $this->organizationId, 'result_files' => \array_map(static fn(RetrieveResponseFile $result): array => $result->toArray(), $this->resultFiles), 'status' => $this->status, 'validation_files' => \array_map(static fn(RetrieveResponseFile $result): array => $result->toArray(), $this->validationFiles), 'training_files' => \array_map(static fn(RetrieveResponseFile $result): array => $result->toArray(), $this->trainingFiles), 'updated_at' => $this->updatedAt];
    }
}
