...
namespace BoldMinded\Carson\Dependency\OpenAI\Responses\Meta;

use BoldMinded\Carson\Dependency\OpenAI\Contracts\MetaInformationContract;
...
final class MetaInformation implements MetaInformationContract
{
     ...
     */
    use ArrayAccessible;
    private function __construct(public ?string $requestId, public readonly MetaInformationOpenAI $openai, public readonly ?MetaInformationRateLimit $requestLimit, public readonly ?MetaInformationRateLimit $tokenLimit)
    {
    }
    /**
    ...
    public static function from(array $headers) : self
    {
        ...
        $openai = MetaInformationOpenAI::from(['model' => $headers['openai-model'][0] ?? null, 'organization' => $headers['openai-organization'][0] ?? null, 'version' => $headers['openai-version'][0] ?? null, 'processingMs' => isset($headers['openai-processing-ms'][0]) ? (int) $headers['openai-processing-ms'][0] : null]);
        if (isset($headers['x-ratelimit-remaining-requests'][0])) {
            $requestLimit = MetaInformationRateLimit::from(['limit' => isset($headers['x-ratelimit-limit-requests'][0]) ? (int) $headers['x-ratelimit-limit-requests'][0] : null, 'remaining' => (int) $headers['x-ratelimit-remaining-requests'][0], 'reset' => $headers['x-ratelimit-reset-requests'][0] ?? null]);
        } else {
            $requestLimit = null;
        }
        if (isset($headers['x-ratelimit-remaining-tokens'][0])) {
            $tokenLimit = MetaInformationRateLimit::from(['limit' => isset($headers['x-ratelimit-limit-tokens'][0]) ? (int) $headers['x-ratelimit-limit-tokens'][0] : null, 'remaining' => (int) $headers['x-ratelimit-remaining-tokens'][0], 'reset' => $headers['x-ratelimit-reset-tokens'][0] ?? null]);
        } else {
            $tokenLimit = null;
        }
        return new self($requestId, $openai, $requestLimit, $tokenLimit);
    }
    /**
     * {@inheritDoc}
     */
