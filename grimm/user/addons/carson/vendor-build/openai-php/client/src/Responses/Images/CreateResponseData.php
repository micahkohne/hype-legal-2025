...
namespace BoldMinded\Carson\Dependency\OpenAI\Responses\Images;
...
final class CreateResponseData implements ResponseContract
{
     ...
     */
    use ArrayAccessible;
    private function __construct(public readonly string $url, public readonly string $b64_json, public readonly ?string $revisedPrompt)
    {
    }
    /**
    ...
    {
        return new self($attributes['url'] ?? '', $attributes['b64_json'] ?? '', $attributes['revised_prompt'] ?? null);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        $data = $this->url !== '' && $this->url !== '0' ? ['url' => $this->url] : ['b64_json' => $this->b64_json];
