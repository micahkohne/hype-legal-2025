...
namespace BoldMinded\Carson\Dependency\OpenAI\Testing\Responses\Concerns;
...
trait Fakeable
{
    ...
    private static function buildAttributes(array $original, array $override) : array
    {
        ...
        foreach ($original as $key => $entry) {
            $new[$key] = \is_array($entry) ? self::buildAttributes($entry, $override[$key] ?? []) : $override[$key] ?? $entry;
            unset($override[$key]);
        }
    ...
    {
        return MetaInformation::from(['openai-model' => ['gpt-3.5-turbo-instruct'], 'openai-organization' => ['org-1234'], 'openai-processing-ms' => ['410'], 'openai-version' => ['2020-10-01'], 'x-ratelimit-limit-requests' => ['3000'], 'x-ratelimit-limit-tokens' => ['250000'], 'x-ratelimit-remaining-requests' => ['2999'], 'x-ratelimit-remaining-tokens' => ['249989'], 'x-ratelimit-reset-requests' => ['20ms'], 'x-ratelimit-reset-tokens' => ['2ms'], 'x-request-id' => ['3813fa4fa3f17bdf0d7654f0f49ebab4']]);
    }
}
