...
namespace BoldMinded\Carson\Dependency\OpenAI\Responses\Audio;
...
final class TranslationResponse implements ResponseContract, ResponseHasMetaInformationContract
{
     ...
     */
    private function __construct(public readonly ?string $task, public readonly ?string $language, public readonly ?float $duration, public readonly array $segments, public readonly string $text, private readonly MetaInformation $meta)
    {
    }
    /**
    ...
    public static function from(array|string $attributes, MetaInformation $meta) : self
    {
        ...
        $segments = isset($attributes['segments']) ? \array_map(fn(array $result): TranslationResponseSegment => TranslationResponseSegment::from($result), $attributes['segments']) : [];
    ...
    {
        return ['task' => $this->task, 'language' => $this->language, 'duration' => $this->duration, 'segments' => \array_map(static fn(TranslationResponseSegment $result): array => $result->toArray(), $this->segments), 'text' => $this->text];
    }
}
