...
use BoldMinded\Carson\Dependency\OpenAI\Enums\FineTuning\FineTuningEventLevel;
...
final class ListJobEventsResponseEvent implements ResponseContract
{
     ...
     */
    use ArrayAccessible;
    private function __construct(public readonly string $object, public readonly string $id, public readonly int $createdAt, public readonly FineTuningEventLevel $level, public readonly string $message, public readonly ?ListJobEventsResponseEventData $data, public readonly string $type)
    {
    }
    /**
    ...
    {
        return ['object' => $this->object, 'id' => $this->id, 'created_at' => $this->createdAt, 'level' => $this->level->value, 'message' => $this->message, 'data' => $this->data?->toArray(), 'type' => $this->type];
    }
}
